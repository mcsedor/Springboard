...
SELECT facid, 
 ...
 FROM country_club.Facilities
 WHERE membercost > 0 
 AND membercost < 0.05 * monthlymaintenance
...
SELECT name,
       monthlymaintenance,
       CASE WHEN monthlymaintenance > 100 THEN 'expensive' ELSE 'cheap' END AS maintenance_category
 FROM country_club.Facilities

/* Q6: You'd like to get the first and last name of the last member(s)
who signed up. Do not use the LIMIT clause for your solution. */

SELECT CONCAT(firstname,' ',surname) AS full_name
 FROM Members
 WHERE joindate =
    (SELECT MAX(joindate)
     FROM Members)
...
SELECT DISTINCT CONCAT( m.firstname, ' ', m.surname ) AS full_name, f.name AS facility
 FROM Bookings b
  JOIN Facilities f ON b.facid = f.facid
  JOIN Members m ON m.memid = b.memid
 WHERE f.name LIKE 'Ten%' AND m.firstname NOT LIKE 'GUEST%'
 ORDER BY 1,2
